...
namespace Greepo
{
    public partial class SearchMunicipioProject : Form
    {
        ...
        private async Task GetProjectsByMunicipio()
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    lblTitle.Text= "Projetos do município de "+ json[0]["municipality_name"].ToString();
                    if (json != null)
                    {
        ...
        private async Task ConfigureChart()
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                HttpResponseMessage response = await client.GetAsync($"http://localhost:2080/projectIdandValues/{_municipioId}");
                if (response.IsSuccessStatusCode)
                {
                    string responseBody = await response.Content.ReadAsStringAsync();
        ...
        {
            SearchMunicipio mun = new SearchMunicipio();
            mun.Show();
            Hide();
        }
    }
